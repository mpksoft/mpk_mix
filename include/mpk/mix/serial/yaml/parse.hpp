/** @file
 * @brief Parse YAML documents into structured data
 *
...
auto parse_value(T& value, YAML::Node const& node, Path& path) -> void;
...
template <typename T>
auto parse_value(std::vector<T>& value, YAML::Node const& node, Path& path)
    -> void;

template <typename T>
auto parse_value(std::optional<T>& value, YAML::Node const& node, Path& path)
    -> void;

template <typename... Ts>
auto parse_value(
    std::variant<Ts...>& value, YAML::Node const& node, Path& path) -> void;
...
auto parse_value(T& value, YAML::Node const& node, Path& path) -> void
{
    try
    {
        parse_simple_value(value, node.as<std::string>());
    }
    catch (std::exception& e)
    {
        throw_<std::invalid_argument>(
            "Failed to parse YAML node at {}: {}", path, e.what());
    }
}
...
auto check_variant_fields(YAML::Node const& node, PathView path) -> void;
...
auto with_child_node(
    ...
    PathItem child_key,
    FuncRef<void(YAML::Node const&)> f,
    Path& parent_path) -> void;

auto node_keys(YAML::Node const& node, PathView path)
    ...
    -> void
{
    if (node.IsNull())
        return value.reset();
    if (node.IsScalar() && node.as<std::string>() == "none")
        return value.reset();
    value = T{};
    ...
    std::variant<Ts...>& value, YAML::Node const& node, Path& path) -> void
{
    ...
    std::string type;
    auto parse_type = [&](YAML::Node const& child_node)
    {
    ...
    auto maybe_parse_value =
        [&]<size_t I, typename T>(
            YAML::Node const& child_node, Const_Tag<I>, Type_Tag<T>)
    {
        if (type != types[I])
            return;
    ...
    auto parse_val = [&](YAML::Node const& child_node)
    {
        [&]<size_t... I>(std::index_sequence<I...>)
        {
...
auto parse_value(T& value, YAML::Node const& node, Path& path) -> void
{
    ...
    {
        auto parse_item = [&](YAML::Node const& value_node)
        {
...
template <typename T>
auto parse_value(T& value, YAML::Node const& node, PathView root = {}) -> void
{
...
template <typename T>
auto parse_value(YAML::Node const& node, Type_Tag<T> = {}, PathView root = {})
    -> T
{
    auto result = T{};
