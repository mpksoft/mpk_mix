/** @file
 * @brief Implementation of YAML document parser
 *
...
namespace mpk::mix::serial::yaml::detail {
    ...
    PathView path) -> void
{
    if (!node.IsMap())
        throw_<std::invalid_argument>(
            "Expected a YAML map node at {} with fields {}",
            path,
    ...
    std::ostringstream s;
    s << "Failed to parse YAML node at " << path << ": ";
    auto const* delim = "";
...
auto check_variant_fields(YAML::Node const& node, PathView path) -> void
{
    if (!node.IsMap())
        throw_<std::invalid_argument>(
            "Expected a YAML map node at {} with fields type, value", path);
    ...
    PathView path) -> void
{
    ...
    if (match_count == 0)
        throw_<std::invalid_argument>(
            "Failed to parse YAML node at {}: type must be one of [{}], "
            "got {}",
            path,
    ...
    if (match_count != 1)
        throw_<std::logic_error>(
            "Failed to parse YAML node at {}: {} is found "
    ...
    -> std::vector<std::string>
{
    if (!node.IsMap())
        throw_<std::invalid_argument>(
            "Expected a YAML map node at {}", path);

    auto result = std::vector<std::string>{};
    for (auto const& pair: node)
        result.push_back(pair.first.as<std::string>());
    return result;
}
